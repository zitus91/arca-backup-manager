    ...
    <div class="card bg-base-100 border border-base-content/5 rounded-xl">
        <div class="card-body p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-5">
                ...
                <div class="form-control">
                    <label class="label pb-1.5">
                        <span class="label-text text-xs font-semibold text-base-content/50 uppercase tracking-wider">{{ __('users.language') }}</span>
                    </label>
                    <select wire:model="locale" class="select select-bordered w-full rounded-xl bg-base-200/30 border-base-content/10 focus:border-primary focus:bg-base-100 transition-colors">
                        <option value="it">{{ __('users.lang_it') }}</option>
                        <option value="en">{{ __('users.lang_en') }}</option>
                    </select>
                    @error('locale') <p class="text-error text-xs mt-1.5">{{ $message }}</p> @enderror
                </div>
            </div>
